...
output "gitops_metadata" {
  ...
  value = merge(
    { for k, v in {
      ...
      } : "apigatewayv2_${k}" => v if var.enable_apigatewayv2
    },
    { for k, v in {
    ...
    { for k, v in {
      ...
      } : "prometheusservice_${k}" => v if var.enable_prometheusservice
    },
    { for k, v in {
    ...
    { for k, v in {
      ...
      } : "eventbridge_${k}" => v if var.enable_eventbridge
    }
  )
}